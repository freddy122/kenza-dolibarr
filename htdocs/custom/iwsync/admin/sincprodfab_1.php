        ...
        foreach($arrIdProd as $idprod){
            ...
            if(is_dir(DOL_DATA_ROOT."/produit/".$prod_object->ref)){
                ...
                foreach($filesprod as $allf){
                    if(strpos($allf, "icon1.") === false && strpos($allf, "icon2.") === false &&  strpos($allf, "thumb") === false){
                        $imageParams[] = $allf;
                    }
                }
...
print '<tr>'
    ...
    . '<td class="titlefield"> '.$form_html->multiselectarray("iw_prod_fab_name", $prod_fab_class->getParentProductFab(),(!empty(GETPOST("iw_prod_fab_name"))?GETPOST("iw_prod_fab_name"):[]),0,0,'',0,"60%").' </td>'
    . '</tr>';
print '<tr>'
    . '<td colspan="2"> '
